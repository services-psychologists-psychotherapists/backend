...
def validate_birthday(data):
    now = timezone.now()
    age = now.year - data.year - ((now.month, now.day) < (data.month, data.day))
    if age > MAX_LIFESPAN:
        raise ValidationError({"birthday": "Укажите корректный год рождения"})
    if age < PSYCHO_MIN_AGE:
        raise ValidationError({"birthday": "Мы работаем с психологами старше 25 лет"})


def validate_started_working(data):
    now = timezone.now().year
    if data.year > now:
        raise ValidationError({"experience": "Некорректно указан опыт работы"})
...
def validate_graduation_year(data):
    try:
        ...
        raise ValidationError(
            {"graduation_year": "Значение поля должно быть вида 'YYYY-YYYY' или 'YYYY'"}
        )
